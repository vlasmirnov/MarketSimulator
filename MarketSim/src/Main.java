...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			FileWriter resultsMarketFileWriter = new FileWriter(
					resultsMarketFile.getAbsoluteFile());
			FileWriter resultsEntitiesFileWriter = new FileWriter(
					resultsEntitiesFile.getAbsoluteFile());
			market.marketBufferedWriter = new BufferedWriter(
					resultsMarketFileWriter);
			market.commoditiesBufferedWriter = new BufferedWriter(
					resultsEntitiesFileWriter);
			market.marketBufferedWriter
					.write("marketpriceA,marketpriceB,marketpriceC,marketpriceD");
			market.commoditiesBufferedWriter
					.write("consumptionA,consumptionB,consumptionC,consumptionD,budgetvariance");
		 ...
		 */
		for (int a = 0; a < 25; a++) {
			newagents.add(new Agent(market, new ProducerPattern(), "Citizen "
					+ 4 * a, c1, 3, 2, 1000d));
			newagents.add(new Agent(market, new ProducerPattern(), "Citizen "
					+ (4 * a + 1), c2, 4, 2, 1000d));
			newagents.add(new Agent(market, new ProducerPattern(), "Citizen "
		...
		for (int a = 0; a < 20; a++) {
			...
			newagents.add(new Agent(market, new TrendPattern(market, days),
					"Speculator " + a, c1, 0, 0, 1000d));
		}
	 ...
	 * @throws IOException
	 */
	public static void displayRoundData(List<RoundData> roundDataList)
			throws IOException {
		...
		for (Commodity commodity : commodities) {
			...
			// Filled in information on commodity prices, let's make a graph
			// based on it now:
			JFrame f = new JFrame();
