...
public class Market {
	...
	/**
	 * Goes through one round. Clears the bids, updates the agents, and
	...
	/**
	 * Prints out the current simulation information TODO: turn datapoints into
	 * csv / excell file to analyze
	 */
	public void printMarketInformation() {
		...
		try {
			marketBufferedWriter.write(marketcsvline.substring(0,
					marketcsvline.length() - 1));
		...
		for (Commodity commodity : commodities) {
			{
				...
				for (Bid buyBid : buyList) {
					for (Bid sellBid : sellList) {
						if (sellBid.quantity != 0
								&& buyBid.price >= sellBid.price) {
							int maxBuyAmount;
							if (buyBid.marginalscalefactor != 1) {
								maxBuyAmount = (int) Math
										.ceil((Math.log(sellBid.price * 1.0
												/ buyBid.price) / Math
												.log(1 / buyBid.marginalscalefactor))) + 1;
							} else {
								maxBuyAmount = buyBid.quantity;
							}
							int maxBuyAmount2 = (int) (buyBid.spendingcap / sellBid.price);
							int buyAmount = Math.min(
									Math.min(maxBuyAmount, maxBuyAmount2),
									sellBid.quantity);
							if (buyAmount > 0) {
								if (buyAmount >= sellBid.quantity) {
									transaction(buyBid.agent, sellBid.agent,
											sellBid.commodity,
											sellBid.quantity, sellBid.quantity
													* sellBid.price);
									buyBid.spendingcap = buyBid.spendingcap
											- sellBid.quantity * sellBid.price;
									buyBid.price = buyBid.price
											* Math.pow(
													(1 / buyBid.marginalscalefactor),
													sellBid.quantity);
									quantitySold = quantitySold
											+ sellBid.quantity;
									newMarketPrice = newMarketPrice
											+ sellBid.quantity * sellBid.price;
									buyBid.quantity = buyBid.quantity
											- sellBid.quantity;
									sellBid.quantity = 0;
									...
									transaction(buyBid.agent, sellBid.agent,
											sellBid.commodity, buyAmount,
											buyAmount * sellBid.price);
									buyBid.spendingcap = buyBid.spendingcap
											- buyAmount * sellBid.price;
									buyBid.price = buyBid.price
											* Math.pow(
									...
									newMarketPrice = newMarketPrice + buyAmount
											* sellBid.price;
									sellBid.quantity = sellBid.quantity
											- buyAmount;
									buyBid.quantity = buyBid.quantity
	 ...
	 * @param price
	 */
	private void transaction(Agent buyer, Agent seller, Commodity commodity,
			int quantity, double price) {
		buyer.budget = buyer.budget - price;
		seller.budget = seller.budget + price;
		buyer.inventory.put(commodity, buyer.inventory.get(commodity)
				+ quantity);
		seller.inventory.put(commodity, seller.inventory.get(commodity)
				- quantity);
		System.out.println(buyer.name + " bought " + quantity + " units of "
				+ commodity.name + " from " + seller.name + " at " + price
				/ quantity + " galactic intracredits each");
	}
